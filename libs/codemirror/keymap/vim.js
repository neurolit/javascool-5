...
(function () {
    ...
    CodeMirror.keyMap["vim-prefix-g"] = {
        "E":countTimes(function (cm) {
            cm.setCursor(moveToWord(cm, word, -1, 1, "start"));
        }),
        "Shift-E":countTimes(function (cm) {
            cm.setCursor(moveToWord(cm, bigWord, -1, 1, "start"));
        }),
        "G":function (cm) {
    ...
    CodeMirror.keyMap["vim-prefix-y"] = {
        "Y":countTimes(function (cm) {
            pushInBuffer("\n" + cm.getLine(cm.getCursor().line + yank));
            yank++;
        }),
        "'":function (cm) {
            cm.setOption("keyMap", "vim-prefix-y'");
    ...
    motions = {
        ...
        'Shift-B':function (cm, times) {
            return moveToWord(cm, bigWord, -1, times);
        },
        'E':function (cm, times) {
            return moveToWord(cm, word, 1, times, 'end');
        },
        'Shift-E':function (cm, times) {
            return moveToWord(cm, bigWord, 1, times, 'end');
        },
        'J':function (cm, times) {
            var cur = cm.getCursor();
